<template>
  <el-card>
    ...
    <el-form ref="apartmentFormRef" :model="formData" :rules="rules" label-width="120px" style="max-width: 1000px"
      status-icon>
      ...
      <el-form-item label="身份证号码" prop="identificationNumber">
        <el-input v-model.trim="formData.identificationNumber" placeholder="请输入身份证号码" />
      </el-form-item>
      <el-form-item label="房间" required>
        <div class="flex-center">
          <el-form-item prop="provinceId">
            <el-select v-model="areaInfo.provinceId" placeholder="请选择省份" clearable @change="provinceChangeCallback"
              @clear="provinceClearCallback">
              <el-option v-for="item in areaInfo.provinceList" :key="item.id" :label="item.name"
                :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="cityId">
            <el-select v-model="areaInfo.cityId" placeholder="请选择城市" clearable @change="cityChangeCallback"
              @clear="cityClearCallback">
              <el-option v-for="item in areaInfo.cityList" :key="item.id" :label="item.name"
                :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="districtId">
            <el-select v-model="areaInfo.districtId" placeholder="请选择区域" clearable @change="districtChangeCallback"
              @clear="districtClearCallback">
              <el-option v-for="item in areaInfo.districtList" :key="item.id" :label="item.name"
                :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="apartmentId">
            <el-select v-model="areaInfo.apartmentId" placeholder="请选择公寓" clearable filterable
              @change="apartmentChangeCallback" @clear="apartmentClearCallback">
              <el-option v-for="item in areaInfo.apartmentList" :key="item.id" :label="item.name"
                :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="roomId">
            <el-select v-model="areaInfo.roomId" placeholder="请选择房间" clearable filterable @change="roomChangeCallback"
              @clear="roomClearCallback">
              <el-option v-for="item in areaInfo.roomList" :key="item.id" :label="item.roomNumber"
                :value="item.id"></el-option>
            </el-select>
          </el-form-item>
        </div>
      ...
      <el-form-item label="开始时间" prop="leaseStartDate" required>
        <el-date-picker v-model="formData.leaseStartDate" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
          placeholder="开始时间" clearable />
      </el-form-item>
      <el-form-item label="结束时间" prop="leaseEndDate" required>
        <el-date-picker v-model="formData.leaseEndDate" type="date" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
          placeholder="结束时间" clearable />
      </el-form-item>
      <el-form-item label="支付方式" prop="paymentTypeIds">
        <el-select style="width: 100%" v-model="formData.paymentTypeId" placeholder="请选择支付方式" clearable>
          <el-option v-for="item in paymentInfoList" :key="item.id" :label="`${item.name} (${item.additionalInfo})`"
            :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="可选租期" prop="leaseTermIds">
        <el-select style="width: 100%" v-model="formData.leaseTermId" placeholder="请选择租期" clearable>
          <el-option v-for="item in leaseTermInfoList" :key="item.id" :label="`${item.monthCount}${item.unit}`"
            :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="租金（元/月）" prop="rent" required>
      ...
      <el-form-item label="备注信息" prop="additionalInfo">
        <el-input v-model.trim="formData.additionalInfo" type="textarea" placeholder="请输入备注信息" />
      </el-form-item>
    </el-form>
