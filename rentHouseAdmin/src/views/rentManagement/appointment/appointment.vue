<template>
  <el-card>
    <ProTable ref="proTable" :dataCallback="dataCallback" :columns="columns" :requestApi="getAppointmentInfoList"
      :initParam="initParam" :stripe="true">
      <!-- 表格操作 -->
      <template #operation="scope">
        <div style="display: flex; gap: 8px;">
          <el-button :type="scope.row.appointmentStatus === AppointmentStatus.WAITING
            ? 'primary'
            : 'info'
            " icon="View" :disabled="scope.row.appointmentStatus !== AppointmentStatus.WAITING"
...
<script setup lang="tsx">
  ...
  // 删除
  const deleteHandle = async (row: AppointmentInfoInterface) => {
    try {
      ...
      await deleteAppointmentInfoById(row.id)
      proTable.value?.getTableList()
