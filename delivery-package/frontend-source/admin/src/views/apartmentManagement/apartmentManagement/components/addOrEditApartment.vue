  ...
  <el-card>
    ...
    <el-form ref="apartmentFormRef" :model="formData" :rules="rules" label-width="120px" style="max-width: 660px"
      ...
      <el-form-item label="所处区域" required>
        <div class="flex-center">
          <el-form-item prop="provinceId">
            <el-select v-model="formData.provinceId" placeholder="请选择省份" clearable @change="provinceChangeCallback"
          ...
          <el-form-item prop="cityId">
            <el-select v-model="formData.cityId" placeholder="请选择城市" clearable @change="cityChangeCallback"
          ...
          <el-form-item prop="districtId">
            <el-select v-model="formData.districtId" placeholder="请选择区域" clearable @change="districtChangeCallback"
      ...
      <el-form-item label="详细地址" prop="addressDetail">
        <el-input v-model="formData.addressDetail" placeholder="请输入详细地址查询" />
      ...
      <el-form-item label="公寓配套" prop="facilityInfoIds">
        <el-select style="width: 100%" v-model="formData.facilityInfoIds" placeholder="请选择公寓配套" multiple clearable>
      ...
      <el-form-item label="公寓标签" prop="labelIds">
        <el-select style="width: 100%" v-model="formData.labelIds" placeholder="请选择公寓标签" multiple clearable>
      ...
      <el-form-item label="公寓杂费" prop="feeValueIds">
        <el-tree-select style="width: 100%" v-model="formData.feeValueIds" placeholder="请选择公寓杂费" :data="feeInfoList"
          multiple clearable node-key="value" :render-after-expand="false"
          @node-click="feeNodeClickHandle"></el-tree-select>
      </el-form-item>
...
<script setup lang="ts">
  ...
  const districtChangeCallback = async () => {
    ...
    if (districtId) {
      ...
      if (district) {
        formData.value.districtName = district.name
      }
    }
    console.log('区域改变')
  }
  ...
  function uploadSuccessHandle(
    ...
    // 兼容两种场景：
    // 1) 新上传文件：item.response.data 为后端返回的图片 URL
    // 2) 编辑回显文件：item.response 为空，但 item.url 已存在
    formData.value.graphVoList =
    ...
    console.log('graphVoList:', formData.value.graphVoList)
  }
  // 根据id获取公寓信息
  ...
  async function addOrUpdateApartmentInfoHandle() {
    try {
      // 验证地区名称是否已设置
      if (!formData.value.provinceName) {
        ElMessage.error('请选择正确的省份')
        return
      }
      if (!formData.value.cityName) {
        ElMessage.error('请选择正确的城市')
        return
      }
      if (!formData.value.districtName) {
        ElMessage.error('请选择正确的区域')
        return
      }
      // 验证图片数据
      if (
        !formData.value.graphVoList ||
        formData.value.graphVoList.length === 0
      ) {
